...
[lme_osci_alpha_ec_hc, lme_osci_table_alpha_ec_hc] = func_LME_slope_on_BLP(struct_hc_reg,'Beta_hi','osci','alpha','EC');
[lme_osci_alpha_ec_sz, lme_osci_table_alpha_ec_sz] = func_LME_slope_on_BLP(struct_sz_reg,'Beta_hi','osci','alpha','EC');

% [lme_frac_beta_ec_hc, lme_frac_table_beta_ec_hc] = func_LME_slope_on_BLP(struct_hc_reg,'Beta_hi','frac','beta','EC');
% [lme_frac_beta_ec_sz, lme_frac_table_beta_ec_sz] = func_LME_slope_on_BLP(struct_sz_reg,'Beta_hi','frac','beta','EC');

%% plotting - alpha osci
f1 = figure('color','w','units','normalized','outerposition',[0.05,0.1,0.9,0.7]);
subplot(121)
hold on
flag_subj = lme_osci_table_alpha_ec_hc.subjID;
subjIDs = unique(lme_osci_table_alpha_ec_hc.subjID);
for n = 1:n_hc
    idx = find(flag_subj==subjIDs(n));
    plot(lme_osci_table_alpha_ec_hc.BLP(idx),lme_osci_table_alpha_ec_hc.beta(idx),'o','LineWidth',2)
end
lsline;
set(gca,'box','on','LineWidth',1,'FontSize',16)
axis square
xlabel('\alpha-BLP^{EC}_{osci} (8-13 Hz)','FontSize',20)
ylabel('\beta_{hi} (20-45 Hz)','FontSize',20)
title('Alpha power vs. spectral slope - Healthy','FontSize',20)

subplot(122)
hold on
flag_subj = lme_osci_table_alpha_ec_sz.subjID;
subjIDs = unique(lme_osci_table_alpha_ec_sz.subjID);
for n = 1:n_sz
    idx = find(flag_subj==subjIDs(n));
    plot(lme_osci_table_alpha_ec_sz.BLP(idx),lme_osci_table_alpha_ec_sz.beta(idx),'o','LineWidth',2)
end
lsline;
set(gca,'box','on','LineWidth',1,'FontSize',16)
axis square
xlabel('\alpha-BLP^{EC}_{osci} (8-13 Hz)','FontSize',20)
ylabel('\beta_{hi} (20-45 Hz)','FontSize',20)
title('Alpha power vs. spectral slope - Schizophrenia','FontSize',20)
