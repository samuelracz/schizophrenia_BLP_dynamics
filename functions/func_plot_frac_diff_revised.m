function [ f ] = func_plot_frac_diff_revised(table_all_corr, table_all_rsn_corr, chanlocs, pstate, flag_sign)
...
vname_mean = 'mean_Beta_bb';
vname_var = 'var_Beta_bb';

table_blp_all = table_all_corr(contains(table_all_corr.meas,vname_mean) | contains(table_all_corr.meas,vname_var),:);
table_sign_blp = table_blp_all(table_blp_all.h_FDR==1,:);

% mean fractal power
if ~isempty(table_sign_blp)
    ind_mean = (contains(table_sign_blp.meas,vname_mean)==1); % select mean fractal power
    p_mean = table_sign_blp(ind_mean,:);
else
    p_mean = [];
end

% variance of fractal power
if ~isempty(table_sign_blp)
    ind_var = (contains(table_sign_blp.meas,vname_var)==1); % select variance of fractal power
    p_var = table_sign_blp(ind_var,:);
else
    p_var = [];
end
...
for iv = 1:length(frac_vars)
    switch iv
        case 1
            stype = 'fractal';
            stype_title = 'Baseline';
            p_blp = p_mean;
            vname = vname_mean;
        case 2
            stype = 'fractal';
            stype_title = 'Fluctuation';
            p_blp = p_var;
            vname = vname_var;
    end

    table_ch = table_blp_all(contains(table_blp_all.meas,vname),:);
    table_ch = sortrows(table_ch,'chID','ascend');

    if strcmp(frac_vars{iv},'mean')
        cblabel = ['\mu(BLP^{' pstate '}_{\it{' stype '}})']; % label for colorbar
    elseif strcmp(frac_vars{iv},'var')
        cblabel = ['\sigma(BLP^{' pstate '}_{\it{' stype '}})']; % label for colorbar
    else
    ...
    switch iv
        case 1
            ...
            if iv == 1
                % ylabel([stype_title ' fractal power' ],'FontSize',20,'visible','on')
            end
            title({'Healthy',[stype_title]},'FontSize',20,'visible','on')
            % title({'Healthy'},'FontSize',20,'visible','on')
            ...
            title({'Schizophrenia',[stype_title]},'FontSize',20,'visible','on')
            % title({'Schizophrenia'},'FontSize',20,'visible','on')
    ...
    if strcmp(frac_vars{iv},'mean')
        str_title = [stype_title ' RSN-wise total fractal power' ];% indicating frequency range in title
        str_ylabel = ['Mean of total fractal power']; % label for colorbar
    elseif strcmp(frac_vars{iv},'var')
        str_title = [stype_title ' of RSN-wise total fractal power' ];% indicating frequency range in title
        str_ylabel = ['Variance of total fractal power']; % label for colorbar
    else
        error('Set vtype to mean or var')
    end
