function [ f ] = func_plot_surrogate_topoplot_revised(table_hc, struct_hc, table_sz, struct_sz, chanlocs, fband, meas, pstate)

if strcmp(meas,'mean')
    str_title = ['\mu(\' fband '-BLP^{' pstate '}_{mixd})'];
elseif strcmp(meas,'var')
    str_title = ['\sigma(\' fband '-BLP^{' pstate '}_{mixd})'];
...
set(gca,'XTick',1:nch,'XTickLabel',struct_hc.chlab,'xlim',[0 nch+1],'FontSize',12)
...
set(gca,'XTick',1:nch,'XTickLabel',struct_sz.chlab,'xlim',[0 nch+1],'FontSize',12)
...
ylim_glob = [min([ylim1(:); ylim2(:)]) max([ylim1(:); ylim2(:)])];
