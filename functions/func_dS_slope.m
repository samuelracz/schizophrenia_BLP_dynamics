function [ output_table ] = func_dS_slope(data_ec, data_eo, chlab, vname, meas)

% Function compare spectral slopes obtained from EYES OPEN and EYES CLOSED
...
for ch = 1:nch % iterate over channels
    ...
    % select variables
    if nch == 1
        v_ec = mean(data_ec.(vname).(meas),2);
        v_eo = mean(data_eo.(vname).(meas),2);
    else
        v_ec = data_ec.(vname).(meas)(:,ch);
        v_eo = data_eo.(vname).(meas)(:,ch);
    end
