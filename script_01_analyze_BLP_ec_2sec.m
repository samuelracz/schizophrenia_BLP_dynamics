...
W = 2; %......................sliding window analysis window size (s)
wstep = 0.25; %..................sliding window analysis step size (s)
...
for subj = 1:ns
    ...
    % saving results
    if ~exist('results_BLP_ec_2sec', 'dir')
       mkdir('results_BLP_ec_2sec')
    end
    subj_saved = subj;
    hdr_saved = hdr;
    fname_ec = [fnames_ec(subj).name(1:end-8) '_BLP_res.mat'];
    save(['results_BLP_ec_2sec/' fname_ec], 'spec_ec', 'plaw_ec', 'results_seg_ec', 'hdr_saved', 'subj_saved');
