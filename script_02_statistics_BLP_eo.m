...
% - illustration of global EYES-OPEN EEG power spectra in HC and SZ
...
% generated first using script_01_analyze_BLP_eo.m
%
% This script only evaluates EYES OPEN data
% 
...
struct_hc_eo = struct();
struct_sz_eo = struct();
struct_hc_rsn_eo = struct();
struct_sz_rsn_eo = struct();

% structures for storing statistical testing outputs
struct_stat_beta_eo = struct();
struct_stat_blp_eo = struct();
struct_stat_rsn_beta_eo = struct();
struct_stat_rsn_blp_eo = struct();

% IRASA analysis
for iv = 1:length(irasa_vars)
    struct_hc_eo.(irasa_vars{iv}) = struct();
    struct_sz_eo.(irasa_vars{iv}) = struct();
    for mv = 1:length(meanvars)
        % channel-level
        struct_hc_eo.(irasa_vars{iv}).(meanvars{mv}) = zeros(n_hc, nch);
        struct_sz_eo.(irasa_vars{iv}).(meanvars{mv}) = zeros(n_sz, nch);
        struct_stat_beta_eo.(irasa_vars{iv}).(meanvars{mv}) = [];
        % RSN-level
        struct_hc_rsn_eo.(irasa_vars{iv}).(meanvars{mv}) = zeros(n_hc, nrsn);
        struct_sz_rsn_eo.(irasa_vars{iv}).(meanvars{mv}) = zeros(n_sz, nrsn);
        struct_stat_rsn_beta_eo.(irasa_vars{iv}).(meanvars{mv}) = [];
    end
end

% BLP analysis
for st = 1:length(spec_types)
    struct_hc_eo.(spec_types{st}) = struct();
    struct_sz_eo.(spec_types{st}) = struct();
    for b = 1:length(bands)
        struct_hc_eo.(spec_types{st}).(bands{b}) = struct();
        struct_sz_eo.(spec_types{st}).(bands{b}) = struct();
        for mv = 1:length(meanvars)
            % channel-level
            struct_hc_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = zeros(n_hc, nch);
            struct_sz_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = zeros(n_sz, nch);
            struct_stat_blp_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = [];
            % RSN-level
            struct_hc_rsn_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = zeros(n_hc, nrsn);
            struct_sz_rsn_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = zeros(n_sz, nrsn);
            struct_stat_rsn_blp_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = [];
        end
    end
end

% for global spectra illustration
struct_hc_eo.spec = struct('subjID',[],'plaw',[]);
struct_hc_rsn_eo.spec = struct('subjID',[],'plaw',[]);
struct_sz_eo.spec = struct('subjID',[],'plaw',[]);
struct_sz_rsn_eo.spec = struct('subjID',[],'plaw',[]);

%% load and structure data

% HC
for subj = 1:n_hc % iterate over HC subjects
    load(['results_BLP_eo/sch_' hc_list{subj} '_eo_BLP_res.mat'])

    % IRASA analysis
    for iv = 1:length(irasa_vars) % iterate over fitting ranges
        v_tmp = zeros(nw, nch);
        for t = 1:nw % iterate over sliding windows
            v_tmp(t,:) = results_seg_eo(t).plaw.(irasa_vars{iv});
        end

        % aggregate over resting-state networks
        v_tmp_rsn = func_aggregate_RSNs_batch(v_tmp, Mr);

        struct_hc_eo.(irasa_vars{iv}).mean(subj,:) = mean(v_tmp,1);
        struct_hc_eo.(irasa_vars{iv}).var(subj,:) = std(v_tmp,[],1);
        struct_hc_rsn_eo.(irasa_vars{iv}).mean(subj,:) = mean(v_tmp_rsn,1);
        struct_hc_rsn_eo.(irasa_vars{iv}).var(subj,:) = std(v_tmp_rsn,[],1);
    end
    ...
    for st = 1:length(spec_types)
        for b = 1:length(bands)
            v_tmp = zeros(nw, nch);
            for t = 1:nw
                freq = results_seg_eo(t).plaw.freq;
                if strcmp(spec_types{st},'mixd')
                    psd = results_seg_eo(t).plaw.mixd;
                    v_tmp(t,:) = log(sum(psd(freq>=freqs(b,1) & freq<freqs(b,2),:),1));
                elseif strcmp(spec_types{st},'frac')
                    psd = results_seg_eo(t).plaw.frac;
                    v_tmp(t,:) = log(sum(psd(freq>=freqs(b,1) & freq<freqs(b,2),:),1));
                elseif strcmp(spec_types{st},'osci')
                    psdm = results_seg_eo(t).plaw.mixd;
                    psdf = results_seg_eo(t).plaw.frac;
            ...
            struct_hc_eo.(spec_types{st}).(bands{b}).mean(subj,:) = mean(v_tmp,1);
            struct_hc_eo.(spec_types{st}).(bands{b}).var(subj,:) = std(v_tmp,[],1);
            struct_hc_rsn_eo.(spec_types{st}).(bands{b}).mean(subj,:) = mean(v_tmp_rsn,1);
            struct_hc_rsn_eo.(spec_types{st}).(bands{b}).var(subj,:) = std(v_tmp_rsn,[],1);
        end
    end

    % store data, compute average power spectrum for RSNs (illustration)
    struct_hc_eo.spec(subj).subjID = fnames_eo(subj).name;
    struct_hc_rsn_eo.spec(subj).subjID = fnames_eo(subj).name;

    % compute average spectra (over sliding windows)
    [plaw, plaw_rsn] = func_average_RSN_results_irasa(results_seg_eo, chlab, Mr);
    struct_hc_eo.spec(subj).plaw = plaw;
    struct_hc_rsn_eo.spec(subj).plaw = plaw_rsn;
end

% SZ
for subj = 1:n_sz % iterate over SZ subjects
    load(['results_BLP_eo/sch_' sz_list{subj} '_eo_BLP_res.mat'])
    for iv = 1:length(irasa_vars) % iterate over fitting ranges
        v_tmp = zeros(nw, nch);
        for t = 1:nw % iterate over sliding windows
            v_tmp(t,:) = results_seg_eo(t).plaw.(irasa_vars{iv});
        ...
        struct_sz_eo.(irasa_vars{iv}).mean(subj,:) = mean(v_tmp,1);
        struct_sz_eo.(irasa_vars{iv}).var(subj,:) = std(v_tmp,[],1);
        struct_sz_rsn_eo.(irasa_vars{iv}).mean(subj,:) = mean(v_tmp_rsn,1);
        struct_sz_rsn_eo.(irasa_vars{iv}).var(subj,:) = std(v_tmp_rsn,[],1);
    end
    ...
    for st = 1:length(spec_types)
        for b = 1:length(bands)
            ...
            struct_sz_eo.(spec_types{st}).(bands{b}).mean(subj,:) = mean(v_tmp,1);
            struct_sz_eo.(spec_types{st}).(bands{b}).var(subj,:) = std(v_tmp,[],1);
            struct_sz_rsn_eo.(spec_types{st}).(bands{b}).mean(subj,:) = mean(v_tmp_rsn,1);
            struct_sz_rsn_eo.(spec_types{st}).(bands{b}).var(subj,:) = std(v_tmp_rsn,[],1);
        end
    end

    % store data, compute average power spectrum for RSNs (illustration)
    struct_sz_eo.spec(subj).subjID = fnames_eo(subj).name;
    struct_sz_rsn_eo.spec(subj).subjID = fnames_eo(subj).name;

    % compute average spectra (over sliding windows)
    [plaw, plaw_rsn] = func_average_RSN_results_irasa(results_seg_eo, chlab, Mr);
    struct_sz_eo.spec(subj).plaw = plaw;
    struct_sz_rsn_eo.spec(subj).plaw = plaw_rsn;
end

% storing channel and RSN labels for convenience
struct_hc_eo.chlab = chlab;
struct_sz_eo.chlab = chlab;
struct_hc_rsn_eo.rsnlab = rsnlab;
struct_sz_rsn_eo.rsnlab = rsnlab;
...
for st = 1:length(spec_types) % iterate over spectra types
    for b = 1:length(bands) % iterate over frequency bands
        for mv = 1:length(meanvars) % iterate over mean and variance
            ...
            % selecting variables
            v_hc = struct_hc_eo.(spec_types{st}).(bands{b}).(meanvars{mv});
            v_sz = struct_sz_eo.(spec_types{st}).(bands{b}).(meanvars{mv});
            ...
            % store outcomes
            struct_stat_blp_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = p_corr;
        end
    end
end
...
for st = 1:length(spec_types)
    for b = 1:length(bands)
        for mv = 1:length(meanvars)
            table_corr = struct_stat_blp_eo.(spec_types{st}).(bands{b}).(meanvars{mv});
            ...
            if isempty(tmp_sign)
                ...
                if isempty(table_sign_blp_unadjusted_eo)
                    table_sign_blp_unadjusted_eo = tmp_sign;
                else
                    table_sign_blp_unadjusted_eo = cat(1,table_sign_blp_unadjusted_eo,tmp_sign);
                end
            end
            ...
            if isempty(tmp_sign_FDR)
                ...
                if isempty(table_sign_blp_eo)
                    table_sign_blp_eo = tmp_sign_FDR;
                else
                    table_sign_blp_eo = cat(1,table_sign_blp_eo,tmp_sign_FDR);
                end
            end
        end
    end
...
% preallocate
table_sign_rsn_blp_eo = [];
table_sign_rsn_blp_unadjusted_eo = [];
...
for st = 1:length(spec_types) % iterate over spectra types
    for b = 1:length(bands) % iterate over frequency bands
        for mv = 1:length(meanvars) % iterate over mean and variance
            ...
            % select variables
            v_hc = struct_hc_rsn_eo.(spec_types{st}).(bands{b}).(meanvars{mv});
            v_sz = struct_sz_rsn_eo.(spec_types{st}).(bands{b}).(meanvars{mv});
            vname = [meanvars{mv} '_' bands{b} '_' spec_types{st}];
            ...
            % store outcomes
            struct_stat_rsn_blp_eo.(spec_types{st}).(bands{b}).(meanvars{mv}) = p_corr;
        end
    end
end
...
for st = 1:length(spec_types)
    for b = 1:length(bands)
        for mv = 1:length(meanvars)
            table_corr = struct_stat_rsn_blp_eo.(spec_types{st}).(bands{b}).(meanvars{mv});
            tmp_sign = table_corr(table_corr.h==1,:);
            tmp_sign_FDR = table_corr(table_corr.h_FDR==1,:);
            ...
            if isempty(tmp_sign)
                ...
                if isempty(table_sign_rsn_blp_unadjusted_eo)
                    table_sign_rsn_blp_unadjusted_eo = tmp_sign;
                else
                    table_sign_rsn_blp_unadjusted_eo = cat(1,table_sign_rsn_blp_unadjusted_eo,tmp_sign);
                end
            end
            ...
            if isempty(tmp_sign_FDR)
                ...
                if isempty(table_sign_rsn_blp_eo)
                    table_sign_rsn_blp_eo = tmp_sign_FDR;
                else
                    table_sign_rsn_blp_eo = cat(1,table_sign_rsn_blp_eo,tmp_sign_FDR);
                end
            end
        end
    end
...
save('miscellaneous/ws_results_BLP_eo.mat', 'chanlocs', 'rsnlab', 'nch', 'nrsn',...
    'struct_hc_eo', 'struct_sz_eo', 'struct_stat_blp_eo',...
    'struct_hc_rsn_eo', 'struct_sz_rsn_eo', 'struct_stat_rsn_blp_eo',...
    'table_sign_blp_eo', 'table_sign_blp_unadjusted_eo',...
    'table_sign_rsn_blp_eo', 'table_sign_rsn_blp_unadjusted_eo')

%% statistics for global spectra
[~, ~, spec_hc, spec_sz, struct_stat_spec] = func_plot_GA_spectra(struct_hc_eo,struct_sz_eo,0,1,0);
...
corr_panss_blp_gen_eo = func_corr_channel_PANSS(table_sign_blp_eo, table_panss, table_conf, 'GEN');
corr_panss_blp_neg_eo = func_corr_channel_PANSS(table_sign_blp_eo, table_panss, table_conf, 'NEG');
corr_panss_blp_pos_eo = func_corr_channel_PANSS(table_sign_blp_eo, table_panss, table_conf, 'POS');
corr_panss_blp_sum_eo = func_corr_channel_PANSS(table_sign_blp_eo, table_panss, table_conf, 'SUM');


%% compute PANSS correlations - BLP, RSN-wise

corr_panss_rsn_blp_gen_eo = func_corr_RSN_PANSS(table_sign_rsn_blp_eo, table_panss, table_conf, 'GEN');
corr_panss_rsn_blp_neg_eo = func_corr_RSN_PANSS(table_sign_rsn_blp_eo, table_panss, table_conf, 'NEG');
corr_panss_rsn_blp_pos_eo = func_corr_RSN_PANSS(table_sign_rsn_blp_eo, table_panss, table_conf, 'POS');
corr_panss_rsn_blp_sum_eo = func_corr_RSN_PANSS(table_sign_rsn_blp_eo, table_panss, table_conf, 'SUM');


%% saving results
save('miscellaneous/ws_correlations_BLP_eo.mat', 'chanlocs', 'rsnlab', 'nch', 'nrsn',...
    'table_panss', 'table_conf', 'z_conf',...
    'corr_panss_blp_gen_eo', 'corr_panss_blp_neg_eo', 'corr_panss_blp_pos_eo', 'corr_panss_blp_sum_eo',...
    'corr_panss_rsn_blp_gen_eo', 'corr_panss_rsn_blp_neg_eo', 'corr_panss_rsn_blp_pos_eo', 'corr_panss_rsn_blp_sum_eo')
