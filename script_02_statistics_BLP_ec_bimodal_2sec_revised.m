...
% manuscript for 2-second sliding window size and 0.25 second step size.
%
%
...
fnames_ec = dir('results_BLP_ec_2sec/*.mat');
...
for subj = 1:n_hc % iterate over HC subjects
    load(['results_BLP_ec_2sec/sch_' hc_list{subj} '_ec_BLP_res.mat'])
...
for subj = 1:n_sz % iterate over SZ subjects
    load(['results_BLP_ec_2sec/sch_' sz_list{subj} '_ec_BLP_res.mat'])
    for iv = 1:length(irasa_vars) % iterate over fitting ranges
        v_tmp = zeros(nw, nch);
        for t = 1:nw % iterate over sliding windows
            % v_tmp(t,:) = results_seg_ec(t).plaw.(irasa_vars{iv});
...
save('miscellaneous/ws_results_BLP_ec_revised_2sec.mat', 'chanlocs', 'rsnlab', 'nch', 'nrsn',...
...
writetable(table_export_blp_exploratory,'miscellaneous/table_blp_exploratory_2sec.xlsx')
writetable(table_export_blp_rsn,'miscellaneous/table_blp_rsn_2sec.xlsx')
...
% writetable(table_panss_rsn_blp_all_ec,'miscellaneous/table_panss_corr_blp_rsn_2sec.xlsx')

%% saving results
save('miscellaneous/ws_correlations_BLP_ec_revised_2sec.mat', 'chanlocs', 'rsnlab', 'nch', 'nrsn',...
